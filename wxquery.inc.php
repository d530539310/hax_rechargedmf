...
$outTradeNo = $_GET['outTradeNo'];     //要查询的订单号
/** 配置结束 */
...
if($result[0] == "SUCCESS"){
    if($_GET['mod']=='crd'){
        $orderdata = C::t('#hax_rechargedmf#hax_rechargedmf_orderlog')->fetch($outTradeNo);
    ...
    // 			'payid'=>$result['alipay_trade_query_response']['buyer_logon_id'],
        ...
        $orderdata= C::t('#hax_rechargedmf#hax_rechargedmf_grporder')->fetch($outTradeNo);
        if($orderdata['state']==0){
